...
<html>
<head>
<title>휴학신청자리스트</title>
</head>
<body>
	<h1><strong>휴학신청자리스트</strong></h1>
	<form>
		<table border="1">
			<tr>
				<th>글번호</th>
				...
				<c:if test="${mainList.size() > 0 }">
					<tr>
						<c:forEach var="item" items="${mainList }">
								...
								<td>
									<input type="button" value="허용" onclick="location.href='/FinalProject/leaveSubmit.do?id=${item.id}&startleavedate=${s_date}&endleavedate=${e_date}&reason=${item.reason }&receivedate=${r_date}'">
									<input type="button" value="기각" onclick="location.href='/FinalProject/leaveRefus.do?id=${item.id}'">
								</td>
							</tr>
						</c:forEach>
					</tr>
			...
			<tr>
				...
				<c:if test="${count > 0}">
				   ...
				   <c:if test="${startPage > pageSize}">
				        <a href="/FinalProject/leaveStudent.do?pageNum=${startPage - pageSize }">[이전]</a>
				   </c:if>
	
				   <c:forEach var="i" begin="${startPage}" end="${endPage}">
				       <a href="/FinalProject/leaveStudent.do?pageNum=${i}">[${i}]</a>
				   </c:forEach>
				
				   <c:if test="${endPage < pageCount}">
				        <a href="/FinalProject/leaveStudent.do?pageNum=${startPage + pageSize}">[다음]</a>
				   </c:if>
				</c:if>
				</td>
			</tr>
			...
			<c:if test="${searchList.size() > 0 }">
				<tr>
					<c:forEach var="item" items="${searchList }">
						<tr>
							...
							<td>
								<input type="button" value="허용" onclick="location.href='/FinalProject/leaveSubmit.do?id=${item.id}&startleavedate=${item.startleavedate }&endleavedate=${item.endleavedate }&reason=${item.reason }&receivedate=${item.receivedate }'" >
								<input type="button" value="기각" onclick="location.href='/FinalProject/leaveRefus.do?id=${item.id}'" >
							</td>
						</tr>
					</c:forEach>
				...
				<c:if test="${count > 0}">
				   ...
				   <c:if test="${startPage > pageSize}">
				        <a href="/FinalProject/leaveStudent.do?pageNum=${startPage - pageSize}&options=${options}&searchContent=${searchContent}">[이전]</a>
				   </c:if>
	
				   <c:forEach var="i" begin="${startPage}" end="${endPage}">
				       <a href="/FinalProject/leaveStudent.do?pageNum=${i}&options=${options}&searchContent=${searchContent}">[${i}]</a>
				   </c:forEach>
				
				   <c:if test="${endPage < pageCount}">
				        <a href="/FinalProject/leaveStudent.do?pageNum=${startPage + pageSize}&options=${options}&searchContent=${searchContent}">[다음]</a>
				   </c:if>
				</c:if>
				</td>
			</tr>
